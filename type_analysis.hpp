...
public:
	...
	void errReadArray(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Attempt to assign user input to array");
	}
	void errReadFile(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Attempt to assign user input to file variable");
	}
	...
	void errArrayID(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Attempt to index a non-array");
	}
	void errArrayIndex(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Bad index type");
	}
	void errFileEq(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Equality operator applied to file variables");
	}
	void errFileOpen(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Opening a non-file variable");
	}
	void errFileClose(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Closing a non-file variable");
	}
